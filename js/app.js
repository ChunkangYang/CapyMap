...
function extractHighwayICs(route) {
  ...
  // Strip false-positive patterns first (street intersections named "○○入口（交差点）" etc.)
  const cleanTxt = (t) => (t || '')
    .replace(/[（(]交差点[)）]/g, '')
    .replace(/入口（[^）]*）/g, '入口')
    .replace(/出口（[^）]*）/g, '出口');
  ...
  // Fallback: scan whole route
  if (!entryCands.length || !exitCands.length) {
    const all = [];
    steps.forEach(s => all.push(...extractFromStep(s)));
    if (!entryCands.length && all.length) entryCands = all.slice(0, 3);
    if (!exitCands.length && all.length)  exitCands  = all.slice(-3);
  }
